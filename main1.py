...
class Birthday():
    ...
    def validate(self, datetime_value):
        if (int(datetime_value[0]) > 0 and int(datetime_value[0]) < 32) \
                and (int(datetime_value[1]) > 0 and int(datetime_value[1]) < 13) \
                and (int(datetime_value[2]) > 1900 and int(datetime_value[2]) < 2023):
            return True
        else:
            return False
    ...
    def add(self, birthday):
        ...
        if self.validate(self.date_value) == True:
...
class AddressBook(UserDict, Record):
    ...
    def process(self, list_of_commands):
        if len(list_of_commands) == 1:
            ...
            # add tom 0965141560 22.08.2000
            # add mike 0637640664 20.07.2001
